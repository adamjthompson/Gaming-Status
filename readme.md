...
* **Master Handoff Grace Period:** If a player is genuinely active on two platforms at once (e.g. playing a Steam game while also idling into a console game), this is how long the platform currently shown on the Master sensor must go stale before a more-recently-started game on another platform is allowed to take over the display. Prevents notification spam from brief connectivity blips flipping which platform is shown back and forth. Default: 300 seconds (5 min). Set to 0 to restore instant hand-off.
